...
let fromUrl = URL.init(fileURLWithPath: NSHomeDirectory()+"/Desktop/"+"input.wav")
let toUrl = URL.init(fileURLWithPath: NSHomeDirectory()+"/Desktop/"+"output.wav")

let input = try! AVAudioFile.init(forReading: fromUrl,commonFormat: .pcmFormatFloat32,interleaved: false)
...
do{
    let output = try AVAudioFile(forWriting: toUrl, settings: settings, commonFormat: .pcmFormatFloat32, interleaved: false)
